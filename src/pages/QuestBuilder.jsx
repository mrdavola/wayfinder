...
export default function QuestBuilder() {
  ...
  const runGeneration = useCallback(async () => {
    ...
    try {
      ...
      // Guard: if AI returned no stages, throw a clear error
      if (!questData?.stages || !Array.isArray(questData.stages)) {
        throw new Error('AI did not return valid project stages. Please try again.');
      }
      ...
      if (generatedQuest.stages?.length) {
        ...
        const { data: savedStages, error: stagesError } = await supabase.from('quest_stages').insert(
          generatedQuest.stages.map((s, i) => ({
            ...
            challenge: s.challenge || s.description || '',
            description: s.challenge || s.description || '',
            tier: s.tier || null,
            required_to_advance: s.required_to_advance || null,
            suggested_creation_mode: s.suggested_creation_mode || null,
            group_adaptation: s.group_adaptation || null,
